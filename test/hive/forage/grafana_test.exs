defmodule Hive.Forage.GrafanaTest do
  ...
  setup do
    ...
    {:ok, product: product, webhook: webhook}
  end
  ...
  test "ingest/3 inserts an alert from a firing payload", ctx do
    ...
    assert {:ok, [%GrafanaAlert{} = alert]} = Grafana.ingest(ctx.product, ctx.webhook, payload)
    ...
    assert alert.product_id == ctx.product.id
  ...
  test "ingest/3 threads firing + resolved into one row, updating status", ctx do
    ...
    assert {:ok, [first]} = Grafana.ingest(ctx.product, ctx.webhook, firing)
    assert first.status == :firing

    assert {:ok, [second]} = Grafana.ingest(ctx.product, ctx.webhook, resolved)
  ...
  test "ingest/3 rejects a payload without alerts", ctx do
    assert {:error, :invalid_payload} =
             Grafana.ingest(ctx.product, ctx.webhook, %{"foo" => "bar"})
  end
  ...
  test "ingest/3 derives a fingerprint when Grafana doesn't send one", ctx do
    ...
    assert {:ok, [alert]} = Grafana.ingest(ctx.product, ctx.webhook, payload)
