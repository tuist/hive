defmodule HiveWeb.ErrorsAPI.EnvelopeControllerTest do
  ...
  describe "POST /api/:project_id/envelope/" do
    test "accepts a valid event envelope and returns an event id", %{conn: conn, project: project} do
      body =
        envelope_body(
      ...
      conn = post(conn, ~p"/api/#{project.id}/envelope/", body)
    ...
    test "rejects requests whose key belongs to a different project", %{conn: conn} do
      ...
      conn = post(conn, ~p"/api/#{other.id}/envelope/", body)
      assert conn.status == 403
    end

    test "rejects requests when ClickHouse is disabled", %{conn: conn, project: project} do
