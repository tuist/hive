defmodule Hive.MCP.Server do
  @moduledoc false

  @instructions """
  When you solve an error retrieved from Hive, include the error issue's `url`
  in the pull request description. Hive uses that link to resolve the error
  automatically after the pull request is merged.
  """

  use EMCP.Server,
    name: "hive",
    version: "0.2.0",
    tools: [
      Hive.MCP.Components.Tools.Whoami,
      Hive.MCP.Components.Tools.ListNotificationPreferences,
      Hive.MCP.Components.Tools.SetNotificationPreference,
      Hive.MCP.Components.Tools.ListProjects,
      Hive.MCP.Components.Tools.GetProject,
      Hive.MCP.Components.Tools.CreateProject,
      Hive.MCP.Components.Tools.UpdateProject,
      Hive.MCP.Components.Tools.DeleteProject,
      Hive.MCP.Components.Tools.ListDomains,
      Hive.MCP.Components.Tools.GetDomain,
      Hive.MCP.Components.Tools.CreateDomain,
      Hive.MCP.Components.Tools.UpdateDomain,
      Hive.MCP.Components.Tools.DeleteDomain,
      Hive.MCP.Components.Tools.LinkProjectDomain,
      Hive.MCP.Components.Tools.UnlinkProjectDomain,
      Hive.MCP.Components.Tools.LinkProjectRepository,
      Hive.MCP.Components.Tools.UnlinkProjectRepository,
      Hive.MCP.Components.Tools.CreateProjectWebhook,
      Hive.MCP.Components.Tools.DeleteProjectWebhook,
      Hive.MCP.Components.Tools.CreateForageItem,
      Hive.MCP.Components.Tools.StartForageItemFlight,
      Hive.MCP.Components.Tools.StartGrafanaAlertFlight,
      Hive.MCP.Components.Tools.ListFlights,
      Hive.MCP.Components.Tools.GetFlight,
      Hive.MCP.Components.Tools.ListPostmortems,
      Hive.MCP.Components.Tools.GetPostmortem,
      Hive.MCP.Components.Tools.CreatePostmortem,
      Hive.MCP.Components.Tools.UpdatePostmortem,
      Hive.MCP.Components.Tools.DeletePostmortem,
      Hive.MCP.Components.Tools.ListPostmortemActionItems,
      Hive.MCP.Components.Tools.GetPostmortemActionItem,
      Hive.MCP.Components.Tools.CreatePostmortemActionItem,
      Hive.MCP.Components.Tools.UpdatePostmortemActionItem,
      Hive.MCP.Components.Tools.DeletePostmortemActionItem,
      Hive.MCP.Components.Tools.ListSpecs,
      Hive.MCP.Components.Tools.GetSpec,
      Hive.MCP.Components.Tools.CreateSpec,
      Hive.MCP.Components.Tools.UpdateSpec,
      Hive.MCP.Components.Tools.DeleteSpec,
      Hive.MCP.Components.Tools.RequestSpecReview,
      Hive.MCP.Components.Tools.ListSpecComments,
      Hive.MCP.Components.Tools.AddSpecComment,
      Hive.MCP.Components.Tools.UpdateSpecComment,
      Hive.MCP.Components.Tools.DeleteSpecComment,
      Hive.MCP.Components.Tools.ListAuditActivities,
      Hive.MCP.Components.Tools.GetAuditActivity,
      Hive.MCP.Components.Tools.ListInferenceProfiles,
      Hive.MCP.Components.Tools.ListInferenceProviders,
      Hive.MCP.Components.Tools.ListInferenceTokens,
      Hive.MCP.Components.Tools.GetInferenceUsage,
      Hive.MCP.Components.Tools.ListDrops,
      Hive.MCP.Components.Tools.GetDrop,
      Hive.MCP.Components.Tools.ListDropDigests,
      Hive.MCP.Components.Tools.GetDropDigest,
      Hive.MCP.Components.Tools.ListErrorIssues,
      Hive.MCP.Components.Tools.GetErrorIssue,
      Hive.MCP.Components.Tools.ListErrorEvents,
      Hive.MCP.Components.Tools.GetErrorEvent,
      Hive.MCP.Components.Tools.ResolveErrorIssue,
      Hive.MCP.Components.Tools.UnresolveErrorIssue,
      Hive.MCP.Components.Tools.IgnoreErrorIssue,
      Hive.MCP.Components.Tools.GetProjectErrorDsn,
      Hive.MCP.Components.Tools.RotateProjectErrorDsn
    ],
    prompts: [
      Hive.MCP.Components.Prompts.WriteSpec
    ]

  def instructions, do: @instructions
end
