defmodule Hive.MCP.Transport.StreamableHTTP do
  @moduledoc false
  ...
  defp request_method(%Plug.Conn{body_params: %{"method" => method}}) when is_binary(method),
    do: method
    ...
    descriptors =
      Enum.map(tools, fn %{"name" => name} = tool ->
        case Map.fetch(modules, name) do
          {:ok, module} -> Hive.MCP.Tool.descriptor(module)
          :error -> tool
        end
      end)
