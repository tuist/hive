defmodule Hive.Meadows.Evolution do
  @moduledoc """
  Builds the meadow evolution input and applies agent-proposed changes.
  """
  ...
  alias Hive.Meadows
  alias Hive.Meadows.Agents.EvolutionAgent
  ...
  def build_input(opts \\ []) do
    ...
    %{
      ...
      current_meadows: current_meadows(),
    ...
    result =
      ...
      |> Enum.reduce(empty_result(), fn change, acc ->
        case apply_change(change) do
          {:created, meadow} ->
            %{acc | created: [meadow | acc.created]}

          {:updated, meadow} ->
            %{acc | updated: [meadow | acc.updated]}
  ...
  defp run_agent(input, opts) do
    ...
    Sessions.run_operation(agent, :evolve_meadows, input, agent_opts)
  end

  defp current_meadows do
    Meadow
  ...
  defp spec_items(limit) do
    ...
    |> Enum.map(fn spec ->
      %{
        ...
        source: "specs",
        meadows: Enum.map(spec.meadows, & &1.name),
  ...
  defp apply_change(change) do
    ...
    cond do
      ...
      not acceptable_name?(name) ->
        {:skipped, :unfit_meadow_name}

      not business_aligned?(name, description) ->
        {:skipped, :outside_tuist_business_domain}

      action == "create" ->
        create_meadow(name, description)

      action == "update" ->
        change
        |> value(:meadow_id)
        |> update_meadow(name, description)
    end
  end

  defp create_meadow(name, description) do
    case meadow_by_normalized_name(name) do
      %Meadow{} ->
        {:skipped, :duplicate_meadow_name}

      nil ->
        case Meadows.create_meadow(%{name: name, description: description}) do
          {:ok, meadow} -> {:created, meadow}
          {:error, changeset} -> {:skipped, {:invalid_meadow, changeset_errors(changeset)}}
        end
    end
  end

  defp update_meadow(nil, _name, _description), do: {:skipped, :missing_meadow_id}
  defp update_meadow("", _name, _description), do: {:skipped, :missing_meadow_id}

  defp update_meadow(meadow_id, name, description) do
    with {:ok, meadow} <- fetch_meadow(meadow_id),
         :ok <- validate_unique_update_name(meadow, name) do
      case Meadows.update_meadow(meadow, %{name: name, description: description}) do
        {:ok, meadow} -> {:updated, meadow}
        {:error, changeset} -> {:skipped, {:invalid_meadow, changeset_errors(changeset)}}
      end
    else
  ...
  defp fetch_meadow(id) do
    with {:ok, uuid} <- Ecto.UUID.cast(id),
         %Meadow{} = meadow <- Repo.get(Meadow, uuid) do
      {:ok, meadow}
    else
      :error -> {:error, :invalid_meadow_id}
      nil -> {:error, :unknown_meadow_id}
    end
  end

  defp validate_unique_update_name(%Meadow{id: meadow_id}, name) do
    case meadow_by_normalized_name(name) do
      nil -> :ok
      %Meadow{id: ^meadow_id} -> :ok
      %Meadow{} -> {:error, :duplicate_meadow_name}
    end
  end

  defp meadow_by_normalized_name(name) do
