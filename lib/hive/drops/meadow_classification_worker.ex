defmodule Hive.Drops.MeadowClassificationWorker do
  @moduledoc """
  Runs the drop-to-meadow classifier asynchronously so the syncer can
  ...
  def perform(%Oban.Job{args: %{"drop_id" => drop_id}}) do
    case MeadowClassification.classify(drop_id) do
      {:ok, _meadow_ids} -> :ok
      {:error, :not_found} -> :ok
